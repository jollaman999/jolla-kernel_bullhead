...
static int mwifiex_init_rxq_ring(struct mwifiex_adapter *adapter)
{
	...
	for (i = 0; i < MWIFIEX_MAX_TXRX_BD; i++) {
		...
		dev_dbg(adapter->dev,
			"info: RX ring: skb=%pK len=%d data=%pK buf_pa=%#x:%x\n",
...
static int mwifiex_pcie_init_evt_ring(struct mwifiex_adapter *adapter)
{
	...
	for (i = 0; i < MWIFIEX_MAX_EVT_BD; i++) {
		...
		dev_dbg(adapter->dev,
			"info: EVT ring: skb=%pK len=%d data=%pK buf_pa=%#x:%x\n",
			skb, skb->len, skb->data, (u32)buf_pa,
			(u32)((u64)buf_pa >> 32));
...
static int mwifiex_pcie_send_data_complete(struct mwifiex_adapter *adapter)
{
	...
	while (((card->txbd_rdptr & reg->tx_mask) !=
		...
		if (skb) {
			dev_dbg(adapter->dev,
				"SEND COMP: Detach skb %pK at txbd_rdidx=%d\n",
				skb, wrdoneidx);
			MWIFIEX_SKB_PACB(skb, &buf_pa);
		       ...
		       struct mwifiex_tx_param *tx_param)
{
	...
	if (!(skb->data && skb->len)) {
		dev_err(adapter->dev, "%s(): invalid parameter <%pK, %#x>\n",
...
static int mwifiex_pcie_process_recv_data(struct mwifiex_adapter *adapter)
{
	...
	while (((wrptr & reg->rx_mask) !=
		...
		dev_dbg(adapter->dev,
			"RECV DATA: Attach new sk_buff %pK at rxbd_rdidx=%d\n",
			skb_tmp, rd_index);
...
mwifiex_pcie_send_boot_cmd(struct mwifiex_adapter *adapter, struct sk_buff *skb)
{
	...
	if (!(skb->data && skb->len)) {
		dev_err(adapter->dev,
			"Invalid parameter in %s <%pK. len %d>\n",
			__func__, skb->data, skb->len);
		return -1;
	}
