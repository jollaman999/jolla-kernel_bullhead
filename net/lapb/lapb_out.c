...
static void lapb_send_iframe(struct lapb_cb *lapb, struct sk_buff *skb, int poll_bit)
{
	...
	lapb_dbg(1, "(%pK) S%d TX I(%d) S%d R%d\n",
...
void lapb_transmit_buffer(struct lapb_cb *lapb, struct sk_buff *skb, int type)
{
	...
	lapb_dbg(2, "(%pK) S%d TX %02X %02X %02X\n",
		 lapb->dev, lapb->state,
...
void lapb_establish_data_link(struct lapb_cb *lapb)
{
	...
	if (lapb->mode & LAPB_EXTENDED) {
		lapb_dbg(1, "(%pK) S%d TX SABME(1)\n", lapb->dev, lapb->state);
		lapb_send_control(lapb, LAPB_SABME, LAPB_POLLON, LAPB_COMMAND);
	} else {
		lapb_dbg(1, "(%pK) S%d TX SABM(1)\n", lapb->dev, lapb->state);
